...
func NewMCTS(root *Node) *MCTS {
	...
	m := &MCTS{}
	m.Root = root
	m.Fixtures = NewFixtures(
		FClubs(m.Clubs),
		FMidweeks(m.Midweeks),
		FXmasWeek(m.XmasWeek),
		FXmasClubs(m.XmasClubs),
		FNewYearWeek(m.NewYearWeek),
		FPresets(m.Presets),
		FDoNotPlays(m.DoNotPlays),
		FPairs(m.Pairs),
		FDesirableMatches(m.DesirableMatches),
		FDesirableHomeAways(m.DesirableHomeAways),
		FMustMeetHomeAways(m.MustMeetHomeAways),
	)
	return m
}
